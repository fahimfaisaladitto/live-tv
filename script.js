...
function parseM3U(txt) {
    txt.split(/\r?\n/).forEach((l, i, a) => {
        if (l.startsWith("#EXTINF")) {
            ...
            const id = slug(name);
            if (stream.startsWith("http") && !channels.find(x => x.id === id)) channels.push({
                id,
                name,
                logo,
                cat,
...
function renderList(arr) {
    ...
    arr.forEach(c => {
        const d = document.createElement("div");
        d.className = "channelItem" + (currentChannel && currentChannel.id === c.id ? " active" : "");
...
searchInput.onkeyup = () => {
    ...
    const base = activeCat === "All" ? channels : channels.filter(c => c.cat === activeCat);
...
tg.href = "https://t.me/fahimfaisaladitto";
tg.target = "_blank";
tg.innerHTML = `<svg viewBox="0 0 0 0" width="0" height="0" fill="#fff"><path d="M9.04 15.84 8.7 19.6c.49 0 .7-.21.96-.46l2.3-2.2 4.77 3.49c.87.48 1.49.23 1.7-.8l3.08-14.44h0c.26-1.22-.44-1.7-1.28-1.39L1.62 9.2c-1.18.46-1.16 1.12-.2 1.42l4.7 1.47L17.4 5.9c.55-.36 1.06-.16.64.2"/></svg>`;
